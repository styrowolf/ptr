...
function RecentsWidget() {
  ...
  const [recentLines, setRecentLines] = useState<LineWithRoute[]>(ToplasPreferences.getRecentLines());
  const [recentStops, setRecentStops] = useState<StopWithoutId[]>(ToplasPreferences.getRecentStops());
      ...
      <View style={styles.border}>
        <TouchableOpacity onPress={() => router.push("/nearbyStops")}>
          <View style={{ flexDirection: "row" }}>
              <FontAwesome5 name="map" size={24} style={{ width: 44, padding: "auto" }} />
        ...
        <TouchableOpacity onPress={() => router.push("/lines/search")}>
          <View style={{ flexDirection: "row" }}>
              <FontAwesome6 name="bus-simple" size={24} style={{ width: 44, paddingLeft: 2, alignSelf: "center" }} />
        ...
        <TouchableOpacity onPress={() => router.push("/stops/search")}>
          <View style={{ flexDirection: "row" }}>
              <FontAwesome name="map-pin" size={24} style={{ width: 44, paddingLeft: 6, alignSelf: "center" }} />
              <Text style={styles.subtitle}>find stops</Text>
          </View>
        </TouchableOpacity>
      </View>
      <Text style={[styles.title, { paddingTop: 10 }]}>Recents <FontAwesome name="history" size={24} /></Text>
      <Divider height={20}/>
      ...
      <ScrollView style={{ paddingBottom: 10, flexGrow: 1 }}>
        { recentStops.map((e, i) => (
          <View key={`${e.stopCode}-${i}`} style={{ paddingVertical: 10 }}>
            <TouchableOpacity onPress={() => { ToplasPreferences.appendRecentStop(e); router.push({ pathname: "/stops/[code]", params: { code: e.stopCode, name: e.stopName, direction: e.direction }}) }}>
  ...
  if (lines.length == 0) {
    ...
    return placeholders.map((placeholder, i) => (
      <View key={`${placeholder}-${i}`} style={{ flexDirection: "row" }}>
        {arrayPad(placeholders, 4, (<View style={{ flex: 1 }}></View>)).map((p: string, i) => (<Text key={i} style={[{ flex: 1 }, styles.linesTableText]}>{p}</Text>))}
      </View>
    ))
  } else {
    const chunkedLines = chunkArray(lines, 4);
    return <>{chunkedLines.map((chunk, i) => (
      <View key={`${chunk.map((e) => e.routeCode).join("-")}-${i}`} style={{ flexDirection: "row" }}>
          {arrayPad(chunk, 4, (<View style={{ flex: 1 }}></View>)).map((line: LineWithRoute, i) => (<Link key={`${line.lineCode}-${i}`} style={[{ flex: 1 }, styles.linesTableText]} onPress={() => ToplasPreferences.appendRecentLine({ lineCode: line.lineCode, routeCode: line.routeCode })} href={{ pathname: "/lines/[code]", params: { code: line.lineCode, routeCode: line.routeCode } }}>{line.lineCode}</Link>))}
      </View>
  ))}</>
  }
}