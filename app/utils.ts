...
export function isOppositeDirection(
  r1: ToplasApi.RouteInfo,
  r2: ToplasApi.RouteInfo,
): boolean {
  ...
  const oppositeByName =
    nameSplits1[0] == nameSplits2[1] &&
    nameSplits1[1] == nameSplits2[0] &&
    !sameName;
...
export function selectClosestFeature(
  ...
  onPressEvent.features.forEach((feature) => {
    ...
    // @ts-ignore
    const dist =
      (onPressEvent.coordinates.longitude - x) ** 2 +
      (onPressEvent.coordinates.latitude - y) ** 2;
