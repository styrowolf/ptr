...
export class ToplasDataProvider {
    static client = new ToplasApiClient({ environment: () => "https://toplas.kurt.town/api"});
    ...
    static async getNearbyStops(lat: number, lon: number): Promise<ToplasApi.NearbyStop[]> {
    ...
    static async getStopAnnouncements(code: number): Promise<ToplasApi.StopAnnouncement[]> {
        return await this.client.stopAnnouncementsLiveStopStopCodeAnnouncementsGet(code);
    }

    static async getLineAnnouncements(code: string): Promise<ToplasApi.LineAnnouncement[]> {
        return await this.client.lineAnnouncementsLiveLineLineCodeAnnouncementsGet(code);
    }
