...
export default function SchedulePage() {
    ...
    const [dayType, setDayType] = useState<ToplasApi.DayType>(ToplasApi.DayType.WorkingDay);
    ...
    return <View style={{ paddingBottom: insets.bottom, paddingTop: 10, paddingHorizontal: 10, flex: 1 }}>
        ...
        <View style={{ flexDirection: "row" }}>
            <TouchableOpacity onPress={() => setDayType(ToplasApi.DayType.WorkingDay)} style={[{ flex: 1 }]}>
                <Text style={[styles.dayButtons, dayType == ToplasApi.DayType.WorkingDay && styles.selected]}>Weekdays</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setDayType(ToplasApi.DayType.Saturday)} style={[{ flex: 1 }]}>
                <Text style={[styles.dayButtons, dayType == ToplasApi.DayType.Saturday && styles.selected]}>Saturday</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setDayType(ToplasApi.DayType.Sunday)} style={[{ flex: 1 }]}>
                <Text style={[styles.dayButtons, dayType == ToplasApi.DayType.Sunday && styles.selected]}>Sunday</Text>
            </TouchableOpacity>
        </View>
        <Divider height={20}/>
        <ScrollView>
            { Object.entries(rows).map(([hour, minutes]) => <ScheduleRow key={`${hour}${minutes}`} hour={hour} minutes={minutes} />) }
        </ScrollView>
    </View>
}

function groupIntoHours(trips: ToplasApi.TimetableTrip[]): Record<number, string[]> {
